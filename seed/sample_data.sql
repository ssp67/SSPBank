-- Sample data for bank schema

-- One branch
INSERT INTO branches (name, address, city, state, zip, country) VALUES
('Main Branch','123 Main St','Metropolis','NY','10001','US');

INSERT INTO employees (branch_id, first_name, last_name, role, email) VALUES
(1,'Alice','Smith','teller','alice.smith@example.com');
...
-- Two personal customers (Canadian)
...
(NULL, 1, 1, 1, 'USD', 1000.00), -- account_number auto-generated
(NULL, 2, 1, 2, 'USD', 500.00);

-- Make a deposit to John's account (credit only)
INSERT INTO transactions (to_account_id, amount, type, status, description, initiated_by_customer_id)
VALUES (1, 250.00, 'deposit', 'posted', 'ATM deposit', 1);
...
VALUES (1, 2, 120.00, 'transfer', 'posted', 'Rent payment', 1);

-- Add a card for John's account
INSERT INTO cards (card_number, account_id, card_type, expiry_date)
VALUES ('4111-1111-1111-1111', 1, 'debit', '2028-12-31');

-- Sample card transaction
INSERT INTO card_transactions (card_id, merchant, amount, currency, status)
VALUES (1, 'Coffee Shop', 4.50, 'USD', 'settled');

-- Product categories and catalogue (Canadian products)
